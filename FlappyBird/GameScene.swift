...
class GameScene: SKScene,SKPhysicsContactDelegate{
    ...
    override func didMove(to view: SKView) {
        ...
        //アイテム音
        let SoundURL = Bundle.main.url(forResource: "itemMusic", withExtension: "mp3")
            do {
                itemSound = try AVAudioPlayer(contentsOf: SoundURL!)
            } catch {
                print("Error")
            }
    ...
    //アイテム
    func setupItem(){
        let itemTexture = SKTexture(imageNamed:"Cherry２")
        itemTexture.filteringMode = .linear
        
        // 移動する距離を計算
        let movingDistance = self.frame.size.width*2 //なぜ＊２？？
        // 画面外まで移動するアクションを作成
        ...
        //画面の右端から
        let View_right_x = self.frame.size.width*0.9
        ...
        let createitemAnimation = SKAction.run({
            ...
            let itemSprite = SKSpriteNode(texture: itemTexture)
                itemSprite.position = CGPoint(x:View_right_x,y:item_y)
        ...
        //次のアイテムを待つ
        let waititemAnimation = SKAction.wait(forDuration: 3)
